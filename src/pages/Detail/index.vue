        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 在加入购物车进行路由跳转之前，需要把你购买的产品的信息通过请求的形式，通知服务器，进行相应的存储 -->
...
export default {
  ...
  methods: {
    ...
    changeActive(attrValue, attr) {
      ...
      //添加选中的属性
      this.attrData[attrValue.saleAttrName]=attrValue.saleAttrValueName
    },
    //加入购物车
    async addShopcar() {
      //addorUpdateShopCart为async关键字，返回的是一个promise函数，我们可以受用awit接收该函数成功或者失败的值
      try {
        //如果没有抛出错误 进行路由跳转
        ...
        //路由跳转---在路由跳转的时候需要带上产品信息(采用会话存储---不持久化)
        sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))
        sessionStorage.setItem('ATTRDATA',JSON.stringify(this.attrData))
        this.$router.push({
          name:'addCartSuccess',
  ...
  watch: {
    skuNum(newValue, oldValue) {
      let regerxObj = /^[0-9]+$/;
      if (regerxObj.test(newValue)) {
        if (newValue < 1) this.skuNum = 1;
      } else {
        this.skuNum = oldValue;
      }
    },
  },
};
